...
function loadSettings() {
  ...
  if (!settings.vibePattern) settings.vibePattern = '0';
...
function sendToWatch(glucose, trend, delta, lastReadSec, graphArray) {
  ...
  msg[KEY_VIBE_PATTERN]    = parseInt(settings.vibePattern)   || 0;
...
Pebble.addEventListener('showConfiguration', function() {
  ...
  var url = CONFIG_URL +
    ...
    '&vibePattern=' + encodeURIComponent(settings.vibePattern || '0') +
